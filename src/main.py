...
from youtube_manager.output_dir_manager import OutputDirManager
from youtube_manager.ydl_opts_builder import YdlOptsBuilder, YdlOptsFormat
from youtube_manager.yt_downloader import YtDownloader
from youtube_manager.yt_playlist_getter import YtPlaylistGetter
...
async def main(logger=AppLogger()):
    try:
        ...
        # プレイリストオブジェクトを取得
        tasks = [YtPlaylistGetter().get(
            playlist_url=url,
            on_error=lambda e: logger.error(f"プレイリストの取得に失敗しました。URL: {url}, エラー: {_e_to_json(e)}")
        ...
        OutputDirManager.crean_dir(
            ...
            on_remove_dir=lambda dir_path: logger.debug(f"空のディレクトリ`{os.path.basename(dir_path)}`を削除しました。")
        )
        ...
        def on_success(movie: Movie, filename: str):
            nonlocal completed
            completed += 1
            logger.info(f"進捗: {completed}/{total}, ファイル: {filename}, URL: {movie.url}")
        def on_error(e: Exception, movie: Movie, filename: str):
            nonlocal completed
            completed += 1
            logger.error(f"進捗: {completed}/{total}, ファイル: {filename}, URL: {movie.url}, エラー: {_e_to_json(e)}")
            ...
            for movie in playlist.movies:
                ...
                if config.SAVE_FORMAT in ["all", "mp4"]:
                    ...
                    if os.path.exists(file_path):
                        # ファイルが存在する場合はスキップ
                        logger.info(f"スキップ: ファイル`{file_name}`は既に存在します。")
                    else:
                        os.makedirs(file_dir, exist_ok=True)
                        logger.info(f"DL準備: ファイル: `{file_name}`, URL: `{movie.url}`")
                        DL_MP4_TASK = asyncio.create_task(YtDownloader().download(
                            movie=movie,
                            filename=file_name,
                ...
                if config.SAVE_FORMAT in ["all", "mp3"]:
                    ...
                    if os.path.exists(file_path):
                        ...
                        os.makedirs(file_dir, exist_ok=True)
                        logger.info(f"DL準備: ファイル: `{file_name}`, URL: `{movie.url}`")
                        DL_MP3_TASK = asyncio.create_task(YtDownloader().download(
                            movie=movie,
                            filename=file_name,
                            builder=YdlOptsBuilder() \
                                .set_output_dir(file_dir) \
        ...
        logger.info(f"ダウンロードが完了しました。実行時間: {main_timer.get_elapsed_time()}秒")
